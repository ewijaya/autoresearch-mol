...
| 19 | agent | nlp | 2 | ~13 hrs | NLP agent replicate 2 |
| 21 | agent | nlp | 3 | ~13 hrs | NLP agent replicate 3 |
| 23 | agent | nlp | 4 | ~13 hrs | NLP agent replicate 4 |
| 25 | agent | nlp | 5 | ~13 hrs | NLP agent replicate 5 |

#### HP-only baseline — all tracks (3 replicates each)
...
| # | Kind | Track | Run | Est. duration | Purpose |
|---|------|-------|-----|---------------|---------|
| 20 | hp_only | smiles | 1 | ~13 hrs | SMILES HP-only replicate 1 |
| 22 | hp_only | smiles | 2 | ~13 hrs | SMILES HP-only replicate 2 |
...
|------|----------|------------|
| **Weekly API rate limits** | High | Already caused 1 pause; expect 2-3 more over 12 days |
| **Disk space (19 GB free)** | Medium | NLP dataset (41 GB) already downloaded; results are small (~50 MB/run); wandb cache cleared |
| **GPU availability** | Low | Single A10G, no contention currently |
| **NLP track convergence** | Unknown | Larger dataset may behave differently than SMILES; first NLP run (task 14) will be informative |
